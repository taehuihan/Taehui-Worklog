...
9. **-COMP 섹션 생성** — 플로우 검증(7·8) 통과 후, 소스 페이지에 **로컬 마스터 컴포넌트**가 있으면 수행 (없으면 skip하고 로그에 "대상 마스터 0" 명기):
   - **`_shared/master-flow/comp-guide.md` 직접 Read** (기억 복원 ❌) 후 그 규칙대로:
     - 대상 = 소스 페이지 안의 마스터만 (공유·중앙 comp 페이지 제외) — §2
     - 마스터를 `[플로우 섹션명]-COMP` 섹션으로 **이동(relocate)**, 플로우 섹션 바로 아래 +500, width 동일 — §1·§3
     - 배치 = 포함 계층 좌→우 트리, 부모별 서브트리, 이름 라벨 없음 — §3
   - **3중 검증**: 수치(width·위치·마진360·겹침0·누락0) + 의미(소속·인스턴스 무결) + **시각(스크린샷 대조 필수)** — §4
