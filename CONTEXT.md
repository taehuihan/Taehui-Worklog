...
| **RealAcademy** | [realacademy/](realacademy/) | **master-flow**: 학습커스텀 `50686:3312`·칭찬저금통 `50744:10173`·학습커스텀-COMP `50809:12120`·**레벨테스트 `50858:12129`/-COMP `50863:16564`(07-10)**. **07-09~10 SSOT 대폭 개정**(공유 guide): 뎁스→"플로우 단계"·밴드 "Flow Step N" / 세그먼트 마커=**모양 기반**(사각형=세그먼트·다이아=분기) / 전진 모달=슬래시 커넥터 / **exit-corridor**(바텀시트 겹침) / **시각확인 필수**. 학습커스텀 3 세그먼트 재구성. **comp-guide.md 신설 + `/master-flow-new` 9단계 연결 완료(07-10)**(컴포넌트 정리 -COMP). **07-10 §2-4 조건 분기 행 신설**(같은 트리거→조건 A/B: 같은 열 다른 행·행순서=소스·조건=서브레이블 괄호 병기·커넥터 라벨❌) + connect-builder 코리도 오작동 수정. 태블릿 1280 편차·ra-live-class 진행 | ⏭️ **`/master-flow-update` comp 반영 규칙 결정·연결 (다음주 2026-07-13~, 태휘 예약)** · 자동노출 모달 출발점 규칙 명문화 |
